  ...
  return (
    <div className="w-full min-h-screen bg-[#fafaff] text-gray-800 font-inter">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-24 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
          ...
          <div className="relative w-full mt-8 md:mt-0">
            <div className="relative rounded-3xl overflow-hidden shadow-2xl">
              ...
              <div className="absolute left-4 sm:left-6 bottom-4 sm:bottom-6 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl px-4 py-3">
                <div className="text-white font-semibold text-sm sm:text-base">ShopEase Picks</div>
                <div className="text-xs sm:text-sm text-white/80">Trendy choices for modern living</div>
              </div>
              ...
              <div className="absolute bottom-3 sm:bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                {images.map((_, i) => (
                  <button
                    ...
                    className={`w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full transition ${currentIndex === i ? "bg-white" : "bg-white/40"}`}
                  />
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 sm:gap-6">
            {categories.length === 0 ? (
              Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="animate-pulse bg-white/60 rounded-2xl h-36 sm:h-40 shadow-sm" />
              ))
            ) : (
              categories.map((cat) => (
                <motion.button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
            {featured.length === 0 ? (
              Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="animate-pulse bg-white rounded-3xl h-64 sm:h-80 shadow-sm" />
              ))
            ) : (
              featured.map((p) => (
                <motion.div
                  ...
                  className="bg-white rounded-3xl p-4 sm:p-5 shadow-lg hover:shadow-2xl transition cursor-pointer flex flex-col"
      ...
      <footer className="bg-gray-900 text-gray-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8">
          <div>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="text-white font-semibold text-base">ShopEase</div>
            </div>
            <p className="text-gray-400 text-sm sm:text-base">Bringing thoughtfully designed products that redefine everyday living.</p>
          </div>
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm sm:text-base mb-3">Subscribe to receive updates, launches & offers.</p>
        ...
        <div className="border-t border-gray-800 text-center py-6 text-gray-500 text-xs sm:text-sm">
